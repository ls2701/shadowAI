...
BATCH_SIZE = 20      # number of domains to classify per LLM call

# ============================================================
# STATIC AI DOMAIN LIST (unchanged, but can be extended manually)
# ============================================================
AI_APPLICATIONS: dict[str, tuple[str, bool]] = {
    # ... (same as before, no need to repeat here)
    # (copy from your original code)
    # I'm omitting the full dict for brevity, but you should keep it as is.
    ...
    # ... etc. (ensure all existing entries remain)
}
...
def add_event_to_state(state: EntityState, row: dict, sensor: str, source: str, user: str | None):
    """Yield completed session/chunks. Fixed short-session bug."""
    ts = row.get("ts_epoch")
...
# ============================================================
# PASS 1: DOMAIN DISCOVERY & CLASSIFICATION (DYNAMIC CACHE)
# ============================================================
...
def classify_domain_batch(domains: list[str]) -> dict[str, dict]:
    ...
    resp = client.chat(
        model=DOMAIN_CLASSIFY_MODEL,   # use the smaller, faster model
...
def load_dynamic_cache() -> dict[str, tuple[str, bool]]:
    """Load persistent domain cache from JSON file. Format: {domain: (app, approved)}."""
    cache = {}
    if DYNAMIC_CACHE_FILE.exists():
        with DYNAMIC_CACHE_FILE.open("r", encoding="utf-8") as f:
            raw = json.load(f)
        for domain, v in raw.items():
            cache[domain] = (v["app"], v["approved"])
    return cache


def save_dynamic_cache(cache: dict[str, tuple[str, bool]]) -> None:
    """Save the dynamic cache back to JSON."""
    out = {k: {"app": v[0], "approved": v[1]} for k, v in cache.items()}
    with DYNAMIC_CACHE_FILE.open("w", encoding="utf-8") as f:
        json.dump(out, f, indent=2, ensure_ascii=False)


def merge_dynamic_cache_into_index() -> None:
    """Merge confirmed AI domains from the dynamic cache into the global AI_DOMAIN_INDEX."""
    dynamic = load_dynamic_cache()
    added = 0
    for domain, (app, approved) in dynamic.items():
        if app:  # only add domains that were classified as AI
            AI_DOMAIN_INDEX[domain] = (app, approved)
            added += 1
    print(f"Merged {added} AI domains from dynamic cache.")


def discover_and_classify_domains(csv_path: Path, workers: int, batch_size: int = BATCH_SIZE) -> None:
    """
    Pass 1: Stream through the CSV, collect all unique unknown domains (not in static list or cache),
    classify them in batches using the LLM, and update the persistent cache.
    """
    static_domains = set(AI_DOMAIN_INDEX.keys())
    dynamic_cache = load_dynamic_cache()
    all_domains = set()

    print("Collecting unique domains from CSV...")
    with csv_path.open(encoding="utf-8-sig", errors="replace", newline="") as fh:
        reader = csv.DictReader(fh, delimiter=",")
        for raw_row in reader:
            norm = {k.strip().lower(): v.strip() for k, v in raw_row.items()}
            event = norm.get("event", "")
            domain = None

            # Reuse the same extraction logic as in normalize_row
            if "device_name=" in event.lower() or "id=firewall" in event.lower():
                domain = extract_kv(event, "dstname")
            elif "mswineventlog" in event.lower() and "packet" in event.lower():
                domain = fast_dns_decode(event)
            elif event.lstrip().startswith("{"):
                try:
                    j = json.loads(event)
                    domain = first_str(j.get("destinationHostname")) or \
                             (j.get("destination") or {}).get("hostname")
                except Exception:
                    pass

            if domain:
                domain = domain.lower().strip().rstrip(".")
                if domain and domain not in static_domains and domain not in dynamic_cache:
                    all_domains.add(domain)

    if not all_domains:
        print("No new domains to classify.")
        return

    domain_list = sorted(all_domains)
    print(f"Found {len(domain_list)} new domains. Classifying in batches of {batch_size}...")

    # Split into batches
    batches = [domain_list[i:i+batch_size] for i in range(0, len(domain_list), batch_size)]

    # Use thread pool to process batches concurrently (bounded by workers)
    with ThreadPoolExecutor(max_workers=min(workers, len(batches))) as executor:
        future_to_batch = {executor.submit(classify_domain_batch, batch): batch for batch in batches}
        for future in as_completed(future_to_batch):
            batch = future_to_batch[future]
            try:
                batch_result = future.result()
                for domain, res in batch_result.items():
                    if res.get("is_ai") and res.get("confidence", 0) >= CONFIDENCE_THRESHOLD:
                        app = res.get("app_name") or domain
                        dynamic_cache[domain] = (app, res.get("approved", False))
                        print(f"  AI: {domain} -> {app} (conf {res['confidence']:.2f})")
                    else:
                        dynamic_cache[domain] = (None, False)
                        print(f"  Non-AI: {domain} (conf {res.get('confidence', 0):.2f})")
            except Exception as e:
                print(f"  Error in batch {batch}: {e}")
                for d in batch:
                    dynamic_cache[d] = (None, False)

    save_dynamic_cache(dynamic_cache)
    print(f"Dynamic cache updated. Total entries: {len(dynamic_cache)}")
...
def main() -> None:
    ...
    ap.add_argument("--classify-first", action="store_true",
                    help="Run Pass 1: discover and classify new domains before the main analysis.")
    args = ap.parse_args()
    ...
    print(f"Domain classify   : batch size {BATCH_SIZE}, confidence threshold {CONFIDENCE_THRESHOLD}")
    print()

    # Optional Pass 1: domain discovery & classification
    if args.classify_first:
        print("=== Pass 1: Domain discovery and classification ===")
        discover_and_classify_domains(args.csv, workers)
        print("=== Pass 1 complete. ===\n")

    # Always merge dynamic cache into the index (so previously learned domains are active)
    merge_dynamic_cache_into_index()

    # Pass 2: main streaming analysis
    process_file(
        args.csv,
        args.output,
        args.review_output,
